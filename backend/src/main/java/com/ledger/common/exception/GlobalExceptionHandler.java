 ...
 * exceptions (including {@link NoResourceFoundException} in Spring Framework
 * 6+)
 ...
 * from the Bean Validation API and is not covered by
...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
         ...
         * method the response is our ApiError JSON rather than the Whitelabel Error
         * Page.
         */
        @Override
         ...
         * Will be exercised from Phase 1 onward when the first request DTO is
         * introduced.
         */
        @Override
        ...
        // -------------------------------------------------------------------------
        // 422 — Invalid ledger entry
        // -------------------------------------------------------------------------

        /**
         * Handles invalid ledger entry data or structure.
         */
        @ExceptionHandler(InvalidLedgerEntryException.class)
        public ResponseEntity<ApiError> handleInvalidLedgerEntry(
                        InvalidLedgerEntryException ex,
                        HttpServletRequest request) {

                String path = request.getRequestURI();

                log.warn("422 Invalid ledger entry on {}: {}", path, ex.getMessage());
        ...
        // -------------------------------------------------------------------------
        // 422 — Unbalanced ledger
        // -------------------------------------------------------------------------

        /**
         * Handles transactions where total debits do not equal total credits.
         */
        @ExceptionHandler(UnbalancedLedgerException.class)
        public ResponseEntity<ApiError> handleUnbalancedLedger(
                        UnbalancedLedgerException ex,
                        HttpServletRequest request) {
